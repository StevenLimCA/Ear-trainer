    ...
    Tone.isPast = function (time) {
      if (time < Tone.context.currentTime) {
        console.warn(
          "Time '" +
            time +
            "' is in the past. Scheduled time must be \u2265 AudioContext.currentTime"
        );
      }
    };
    ///////////////////////////////////////////////////////////////////////////
  ...
  Module(function (Tone) {
    ...
    Tone.TransportRepeatEvent.prototype._createEvents = function () {
      ...
      if (
        ...
        this._nextId = this.Transport.scheduleOnce(
          ...
          Tone.TransportTime(this._nextTick, "i")
        );
      }
    };
    /**
    ...
    Tone.TransportRepeatEvent.prototype._restart = function () {
      ...
      if (ticks > this.time) {
        this._nextTick =
          this.time +
          Math.ceil((ticks - this.time) / this._interval) * this._interval;
      }
  ...
  Module(function (Tone) {
    ...
    Tone.BufferSource.prototype.start = function (
      ...
      if (this.buffer.loaded) {
        ...
        if (fadeInTime > 0) {
          ...
          if (this.curve === "linear") {
            ...
            this._gainNode.gain.setTargetAtTime(
              this._gain,
              time,
              this._gainNode.gain.getTimeConstant(fadeInTime)
            );
          }
        } else {
        ...
        var computedDur = this.toSeconds(
          Tone.defaultArg(duration, this.buffer.duration - offset)
        );
    ...
    Tone.BufferSource.prototype.stop = function (time, fadeOutTime) {
      if (this.buffer.loaded) {
        ...
        if (this._stopTime === -1 || this._stopTime > time) {
          ...
          this._stopTime = time;
          //the fadeOut time
          fadeOutTime = this.toSeconds(
            Tone.defaultArg(fadeOutTime, this.fadeOut)
          );
          //set a new one
          var heldDuration = Math.min(
            time - this._startTime - this.fadeIn - this.sampleTime,
          ...
          if (fadeOutTime > this.sampleTime) {
            ...
            if (this.curve === "linear") {
              ...
              this._gainNode.gain.setTargetAtTime(
                0,
                startFade,
                this._gainNode.gain.getTimeConstant(fadeOutTime)
              );
            }
          } else {
            this._gainNode.gain.setValueAtTime(0, time);
